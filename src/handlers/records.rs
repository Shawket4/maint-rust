...
async fn create_record(
    ...
    if inp.performed_at > Utc::now() + chrono::Duration::hours(24) {
        return Err(ApiError::BadRequest(
            "performed_at cannot be more than 24h in the future".into(),
        ));
    }
...
async fn update_record(
    ...
    if new_performed_at > Utc::now() + chrono::Duration::hours(24) {
        return Err(ApiError::BadRequest("performed_at cannot be more than 24h in the future".into()));
    }
